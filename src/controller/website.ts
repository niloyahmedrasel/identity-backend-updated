...
export class WebsiteController {
  async createWebsite(req: Request, res: Response) {
    try {
      const { businessId, title, logo, domain, templateId, pricePloicy, amount, primaryUrl } = req.body;
      ...
      res.status(200).json({ message: "Website created successfully", data: response });
    } catch (error) {
  ...
  async getbyBusinessId(req: Request, res: Response):Promise<any> {
    try {
      ...
      const response = await websiteService.getWebsitesByBusinessId(new Types.ObjectId(businessId));
      res.status(200).json({ message: "Websites retrieved successfully", data: response });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred";
      res.status(500).json({ message: errorMessage });
  ...
  async updateWebsite(req: Request, res: Response):Promise<any> {
    try {
      ...
      const response = await websiteService.updateWebsite(new Types.ObjectId(websiteId), updateData);

      if (!response) {
        return res.status(404).json({ message: "Website not found" });
      }

      res.status(200).json({ message: "Website updated successfully", data: response });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred";
      res.status(500).json({ message: errorMessage });
    }
  ...
  async deleteWebsite(req: Request, res: Response):Promise<any> {
    try {
      ...
      const response = await websiteService.deleteWebsite(new Types.ObjectId(websiteId));

      if (!response) {
        return res.status(404).json({ message: "Website not found" });
      }

      res.status(200).json({ message: "Website deleted successfully", data: response });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred";
      res.status(500).json({ message: errorMessage });
    }
