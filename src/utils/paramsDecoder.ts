import { PaginationData } from "./api.response";
...
export const FilterParamsDecoder = (filters: string) => {
    try {
        ...
        const processOrFilters = (filterGroup: any[]) => {
            let query: any = {};
        ...
        const processAndFilters = (filterGroup: any[]) => {
            let queryArray: any[] = [];
        ...
        if (Array.isArray(decoded[0][0])) {
            // OR operation
            filterQuery['$or'] = decoded.map((orGroup: any[]) => processOrFilters(orGroup));
        } else {
            // AND operation
            filterQuery['$and'] = processAndFilters(decoded);
        }
...
export const SortParamsDecoder = (sort: string): { [key: string]: 1 | -1 }| undefined => {
    try {
